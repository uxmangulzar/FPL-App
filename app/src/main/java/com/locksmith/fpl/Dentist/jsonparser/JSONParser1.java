package com.locksmith.fpl.Dentist.jsonparser;
...
public class JSONParser1 {
    ...
    public JSONParser1(Context context, JSONObject response) {
        try {
            this.context = context;
            jObj = response;
            SUCCESS = getJsonString(jObj, TAG_SUCCESS);
            MESSAGE = html2text(getJsonString(jObj, TAG_MESSAGE));
            prefrence = SharedPrefrence1.getInstance(context);
            if (SUCCESS.equals("0")) {
                RESULT = false;

            } else if (SUCCESS.equals("3")) {
                prefrence.clearAllPreferences();
                Intent intent = new Intent(context, SignInActivityForTechnician.class);
